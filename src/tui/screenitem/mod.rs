...
use crate::core::{
    config::{DesktopConfig, DesktopInfo, GlobalConfig},
    desktop::DesktopFile,
    pattern::PatternFile,
...
impl ScreenItem {
    ...
    pub fn edit_file(path: &String) {
        match env::var("EDITOR") {
			Ok(value) => if value.is_empty() {
				warn!("Env var |$EDITOR| is empty, using |nano| instead |(try exporting env var FILE_EXPLORER in your shell config)|")
			},
			Err(_) => warn!("Could not found env var |$EDITOR|, using |nano| instead |(try exporting env var FILE_EXPLORER in your shell config)|")
		}
    ...
    pub fn apply(
        ...
        desktop_config: &mut Option<DesktopConfig>,
    ) {
        match self {
    ...
    pub fn is_active(
        ...
        match self {
            ScreenItem::Desktop(d) => match global_config.get_current_desktop() {
                Some(current_desktop) => d.get_name() == current_desktop.get_name(),
                None => false,
            },
            ScreenItem::Theme(t) => match global_config.get_current_theme() {
            ...
            ScreenItem::Extra(e) => match desktop_config {
    ...
    fn apply_theme(
        theme: &ThemeFile,
        global_config: &mut GlobalConfig,
        desktop_config_opt: &mut Option<DesktopConfig>,
    ) {
    ...
    fn apply_desktop(next_desktop: &DesktopFile, global_config: &mut GlobalConfig) {
        ...
        let theme = match next_desktop_config.get_default_theme() {
            ...
            None => {
                error!(
                    "There is no |default theme| specified in desktop |{}|",
                    next_desktop.get_name()
                );
                return;
            }
        };
