...
impl<'a> ListWidget<'a> {
    pub fn new(
        stateful_list: &StatefulList<ScreenItem>,
        ...
        let default_active_style = Style::default()
            .add_modifier(Modifier::BOLD)
            .add_modifier(Modifier::DIM);
        let highlight_active_style = Style::default().add_modifier(Modifier::BOLD);

        let default_theme_style = Style::default()
            .add_modifier(Modifier::BOLD)
            .add_modifier(Modifier::DIM)
            .fg(Color::Yellow);
        let highlight_default_theme_style = Style::default()
            .add_modifier(Modifier::BOLD)
            .fg(Color::Yellow);

        let mut items: Vec<ListItem> = vec![];
        for (it, screen_item) in stateful_list.get_items().iter().enumerate() {
            let state_color = match (
                stateful_list.get_active_text_color(),
                stateful_list.get_inactive_text_color(),
            ) {
                (Some(active), Some(inactive)) => {
                    if screen_item.is_active(global_config, desktop_config) {
                        *active
                    } else {
                        *inactive
                    }
            ...
            let (name_style, active_style, default_theme_style) =
                match stateful_list.get_state().selected() {
                    Some(idx) => {
                        if idx == it {
                            (
                                highlight_name_stlye,
                                highlight_active_style.fg(state_color),
                                highlight_default_theme_style,
                            )
                        } else {
                            (
                                default_name_style,
                                default_active_style.fg(state_color),
                                default_theme_style,
                            )
                        }
                    }
            ...
            let (name, default_theme, active_text, arrows) = Self::get_item_text(
                it,
                screen_item,
                stateful_list,
                global_config,
    ...
    fn get_item_text(
        ...
        let mut name = if *stateful_list.get_alignment() {
            format!("   {:<20} ", name_str)
        } else {
            format!("   {} ", name_str)
        };
        match stateful_list.get_state().selected() {
            Some(idx) => {
                if idx == it {
                    arrows = if idx == 0 {
                        "↓".to_string()
                    } else if idx + 1 == stateful_list.get_length() {
                        "↑".to_string()
                    } else {
                        "↓ ↑".to_string()
                    };

                    name = if *stateful_list.get_alignment() {
                        format!(" ‣ {:<20} ", name_str)
                    } else {
                        format!(" ‣ {} ", name_str)
                    }
