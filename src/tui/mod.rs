...
use crate::tui::{
    appstate::{AppState, Popup, Screen},
    screenitem::ScreenItem,
    widgets::{HelpWidget, InfoWidget, ListWidget, LoggerWidget, LogoWidget},
};
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<Stdout>>) {
    ...
    loop {
        ...
        if !manage_input(&mut app_state, terminal) {
            break;
        }
    }
...
fn manage_input(
    ...
    if let Event::Key(key) = event::read().unwrap() {
        match key.code {
            ...
            KeyCode::Esc => match current_popup {
                Some(p) => {
            ...
            KeyCode::Down | KeyCode::Char('s') | KeyCode::Char('S') => match current_popup {
                Some(p) => {
                    let popup_list = popups.get_mut(p).unwrap();
                    popup_list.next()
                }
            ...
            KeyCode::Up | KeyCode::Char('w') | KeyCode::Char('W') => match current_popup {
                Some(p) => {
                    let popup_list = popups.get_mut(p).unwrap();
                    popup_list.previous()
                }
            ...
            KeyCode::Tab => {
                if *current_popup == None {
                    let screen = if *app_state.get_mut_screen() == Screen::Desktop {
                        Screen::Theme
                    } else {
                        Screen::Desktop
                    };
            ...
            KeyCode::Char('f') | KeyCode::Char('F') => {
                ...
                if current_list == LEFT_LIST {
                    lists
                        .get_mut(LEFT_LIST)
                        .unwrap()
                        .remove_fav(&item, global_config)
                } else {
                    lists
                        .get_mut(LEFT_LIST)
                        .unwrap()
                        .add_fav(&item, global_config);
                }
            }
            KeyCode::Char('i') | KeyCode::Char('I') => match current_popup {
                Some(Popup::Info) => {
                    lists[LEFT_LIST].next();
                    *current_popup = None
                }
...
fn draw_ui(f: &mut Frame<CrosstermBackend<io::Stdout>>, app_state: &mut AppState) {
    ...
    f.render_stateful_widget(
        widget_list_1.get_widget(),
        h_box[0],
        lists[0].get_mut_state(),
    );
    f.render_stateful_widget(
        widget_list_2.get_widget(),
        h_box[1],
        lists[1].get_mut_state(),
    );
    ...
    if *current_popup == Some(Popup::Extras) {
        ...
        f.render_widget(Clear, area);
        f.render_stateful_widget(
            extras_widget.get_widget(),
            area,
            extras_list.get_mut_state(),
        );
    }
