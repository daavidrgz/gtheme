...
impl UserConfigDto {
    fn new() -> Self {
        ...
        let mut file = match File::open(&path) {
            ...
            Err(e) => {
                warn!(
                    "Could not open user settings, using default settings: |{}|",
                    e
                );
        ...
        match file.read_to_string(&mut content) {
            ...
            Err(e) => {
                error!(
                    "Could not read user settings, using default settings: |{}|",
                    e
                );
                let config = Self::default();
                config.save();
        ...
        match toml::from_str(&content) {
            ...
            Err(e) => {
                error!(
                    "Could not parse user settings, using default settings: |{}|",
                    e
                );
                return Self::default();
            }
...
impl UserConfig {
    ...
    pub fn set_property(&mut self, property: &str, value: &str) {
        self.properties
            .insert(String::from(property), String::from(value));
